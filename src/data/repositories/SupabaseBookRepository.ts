...
export class SupabaseBookRepository implements IBookRepository {
  ...
  private mapToBook(bookData: any): Book {
    return {
      id: bookData.id,
  ...
  async getRandomBook(excludeBookIds?: string[]): Promise<Book | null> {
    let query = supabase
      .from('books')
      .select('*')
      .limit(100); // Prendiamo 100 libri e ne scegliamo uno random client-side
  ...
  async incrementViewCount(bookId: string): Promise<void> {
    const { error } = await supabase.rpc('increment_book_view', { book_id: bookId });
  ...
  async incrementClickCount(bookId: string): Promise<void> {
    const { error } = await supabase.rpc('increment_book_click', { book_id: bookId });
